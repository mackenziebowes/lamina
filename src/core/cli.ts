...
export async function runCLI(argv = Bun.argv.slice(2)) {
	...
	if (["-h", "--help"].includes(name)) {
		...
		multiLog.push({
			t: "About",
			m: "lmd (Lamina Mark Down) is a tool for transforming markdown to static content meant for use with component-based style systems.",
		});
		multiLog.push({
			t: "Commands",
		...
		multiLog.push({
			t: "More Info",
			m: "See https://lamina.gesenaworks.com for more detailed instructions",
		});
